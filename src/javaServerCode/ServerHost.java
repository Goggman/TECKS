...
public class ServerHost {
	...
	void setProperties(){
		...
		properties.get("help").put("text", "TODO - add helptext, useful methods and commands, etc");
	}
	...
	public class ClientHandler implements Runnable{
		...
		void parse_msg(String payload){
			...
			chat.add(returnToClients);
			Iterator clients = ((HashMap)((HashMap)properties.get("chatrooms").get(getChat())).get("users")).keySet().iterator();
			while(clients.hasNext()){
				ClientHandler next = (ClientHandler)clients.next();
				if (next != this){
					((ClientHandler)clients.next()).getOut().println(returnToClients);
		...
		void parse_history(String payload){
			...
			ArrayList chatlog = (ArrayList)((HashMap)properties.get("chatrooms").get(this.chatroom)).get("log"); // print every message received so far
			for(int x=0;x<chatlog.size();x++){
				out.println(chatlog.get(x));
			}
